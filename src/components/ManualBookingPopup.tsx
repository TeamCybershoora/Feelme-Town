...
export default function BookingPopup({ isOpen, onClose, isManualMode = false, onSuccess, userInfo }: BookingPopupProps) {
  ...
  useEffect(() => {
    if (isBookingSuccessful && bookingResult) {
      ...
      autoCloseTimerRef.current = window.setTimeout(() => {
        ...
        // Only redirect to theater page if not editing from admin and not manual booking
        if (!wasEditingBooking && !isManualMode) {
          router.push('/theater');
        }
      }, 2000);
    }
  ...
  // Handle redirect after successful manual booking
  useEffect(() => {
    if (isBookingSuccessful && bookingResult && isManualMode && userInfo) {
      ...
      const redirectTimer = setTimeout(() => {
        ...
        let redirectPath = '/theater'; // Default fallback
        
        if (userInfo.type === 'admin') {
          redirectPath = '/Administrator/bookings';
        } else if (userInfo.type === 'staff') {
          redirectPath = '/management/bookings';
        }
