...
export default function AdminDashboard({ stats, recentBookings, onRefresh, refreshing, lastUpdated, currentTime }: AdminDashboardProps) {
  ...
  const handleClosePaymentMethodModal = () => {
    if (paymentUpdatingId !== null && bookingPendingPayment && paymentUpdatingId === String(bookingPendingPayment.originalBookingId || (bookingPendingPayment as any).bookingId || bookingPendingPayment.id).replace(/^#/, '')) {
      return;
    }
    setIsPaymentMethodModalOpen(false);
    setBookingPendingPayment(null);
