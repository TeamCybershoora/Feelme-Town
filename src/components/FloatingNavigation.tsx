...
const FloatingNavigation = () => {
  ...
  const handleFeedbackSubmit = (e: React.FormEvent) => {
    ...
    setFeedbackForm({ name: '', email: '', phone: '', avatar: '', socialHandle: '', message: '', rating: 0 });
    setSelectedAvatar('');
    setUploadedFileName('');
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        setFeedbackForm(prev => ({ ...prev, avatar: result }));
      };
  ...
  const selectGenderAvatar = (gender: 'male' | 'female') => {
    ...
    console.log('Selected avatar:', randomAvatar); // Debug log
    setSelectedAvatar(randomAvatar);
    setFeedbackForm(prev => ({ ...prev, avatar: randomAvatar }));
    setUploadedFileName(`Random ${gender} avatar`);
