...
export default function OrderFoodPage() {
  ...
  const removeOrderedItem = async (idToRemove?: string) => {
    ...
    try {
      ...
      if (data.booking?.dynamicServiceItems && typeof data.booking.dynamicServiceItems === 'object') {
        setOrderedByService(data.booking.dynamicServiceItems as Record<string, FoodItem[]>);
      }
      const newOrdered = (data.booking?.dynamicServiceItems?.selectedFood || []) as FoodItem[];
      setOrderPlaced(newOrdered.length > 0);
  ...
  const fetchBookingByTicket = async (overrideTicket?: string) => {
    ...
    try {
      ...
      setServiceDrafts({});
      setItems([]);
      if (b.dynamicServiceItems && typeof b.dynamicServiceItems === 'object') {
        setOrderedByService(b.dynamicServiceItems as Record<string, FoodItem[]>);
      } else {
        setOrderedByService({});
      }
      // If booking already has food orders, show them first and hide menu until user chooses Add More Items
      const hasFoodOrders = Array.isArray((b as any).dynamicServiceItems?.selectedFood) &&
        (b as any).dynamicServiceItems.selectedFood.length > 0;
  ...
  const handleAddItem = () => {
    ...
    setItems((prev) => [...prev, { id, name, price, quantity: qty }]);
    setNewItemName('');
    setNewItemPrice('');
    setNewItemQty('1');
  };
    ...
    const loadMenu = async () => {
      try {
        ...
        if (defaultServiceKey && !selectedServiceId) {
          ...
          setItems(serviceDrafts[defaultServiceKey] ? [...serviceDrafts[defaultServiceKey]] : []);
        }
        setMenuItems(normalized.length ? normalized : MENU_ITEMS);
      } catch (e) {
        setMenuItems(MENU_ITEMS);
