...
export async function GET() {
  try {
    ...
    // Total Bookings = (confirmed + manual) - cancelled bookings
    const allActiveBookings = [...confirmedBookings, ...allManualBookings];
    ...
    const stats = {
      ...
      allBookings: {
        today: Math.max(0, (onlineBookingsStats.today + manualBookingsStats.today) - cancelledBookingsStats.today),
        thisWeek: Math.max(0, (onlineBookingsStats.thisWeek + manualBookingsStats.thisWeek) - cancelledBookingsStats.thisWeek),
        thisMonth: Math.max(0, (onlineBookingsStats.thisMonth + manualBookingsStats.thisMonth) - cancelledBookingsStats.thisMonth),
        thisYear: Math.max(0, (onlineBookingsStats.thisYear + manualBookingsStats.thisYear) - cancelledBookingsStats.thisYear),
        total: Math.max(0, (onlineBookingsStats.total + manualBookingsStats.total) - cancelledBookingsStats.total)
      },
      activeHalls: {
